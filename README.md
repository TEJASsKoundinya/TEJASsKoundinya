- 👋 Hi, I'm @TEJASsKoundinya
- 🔐 Cyber Security specialist working across IAM, Cloud, and Network Security — with a growing focus on AI x Security R&D
- 👀 Talk to me about Cyber and AI
- 📊 For my AI/ML projects follow me @ [Kaggle](https://www.kaggle.com/tejassurajkoundinya)
- 📫 Reach me at @ [LinkedIn](https://www.linkedin.com/in/tejas-suraj-koundinya-081443191/)
- 🌌 my @ [CV](https://mycv-tsk.pages.dev/)

---

## 🔐 Cybersecurity Skills

**Identity & Access Management (IAM/IGA)**

![Okta](https://img.shields.io/badge/Okta-007DC1?style=for-the-badge&logo=okta&logoColor=white)
![Okta Workflows](https://img.shields.io/badge/Okta%20Workflows-007DC1?style=for-the-badge&logo=okta&logoColor=white)
![Microsoft Entra ID](https://img.shields.io/badge/Microsoft%20Entra%20ID-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)
![SailPoint](https://img.shields.io/badge/SailPoint-1F3352?style=for-the-badge&logoColor=white)
![IGA](https://img.shields.io/badge/IGA-4B0082?style=for-the-badge&logoColor=white)

- Identity Governance & Administration (IGA)
- Okta – SSO, MFA, lifecycle management
- Okta Workflows – no-code identity automation & orchestration
- Microsoft Entra ID (Azure AD) – conditional access, provisioning
- SailPoint – identity security, access certification, role management

**Cloud Security**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)

- AWS security fundamentals (IAM policies, security groups, monitoring)
- Azure security (Entra ID, Defender, security posture management)

**Network Security & SOC**

![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge&logo=wireshark&logoColor=white)
![SIEM](https://img.shields.io/badge/SIEM-4B0082?style=for-the-badge&logoColor=white)

- Wireshark – packet analysis & network traffic investigation
- SIEM monitoring & log analysis for threat detection
- Security operations center (SOC) workflows and incident triage

---

## 🤖 AI / ML Skills

**LLMs & GenAI Platforms**

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Claude](https://img.shields.io/badge/Anthropic%20Claude-D97757?style=for-the-badge&logoColor=white)
![Gemini](https://img.shields.io/badge/Google%20Gemini-8E75B2?style=for-the-badge&logo=googlegemini&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white)
![Azure AI Foundry](https://img.shields.io/badge/Azure%20AI%20Foundry-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)

- Building & orchestrating with OpenAI, Claude, and Gemini APIs
- LangChain for chaining LLM workflows and tool use
- Azure AI Foundry for enterprise-grade model deployment & orchestration

**AI for Security**

- Anomaly detection and threat modeling with ML and AI soc
- Cyber ⇄ AI R&D – AI-for-Cyber (ML-driven threat detection) and Cyber-for-AI (securing AI systems/pipelines)

**Python ML Stack**

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)
![pandas](https://img.shields.io/badge/pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)

- scikit-learn & pandas for data-driven analysis and modeling

**Enterprise AI Platforms**

![Glean](https://img.shields.io/badge/Glean-6A5ACD?style=for-the-badge&logoColor=white)
![Copilot Studio](https://img.shields.io/badge/Copilot%20Studio-0078D4?style=for-the-badge&logo=microsoft&logoColor=white)
![Replit](https://img.shields.io/badge/Replit-F26207?style=for-the-badge&logo=replit&logoColor=white)

- Glean – enterprise search & AI assistant
- Microsoft Copilot Studio – custom copilots/agents
- Replit – rapid AI-assisted prototyping & development

**Custom SLM & RAG Systems**

![RAG](https://img.shields.io/badge/RAG%20Systems-008080?style=for-the-badge&logoColor=white)
![SLM](https://img.shields.io/badge/Custom%20SLMs-2E8B57?style=for-the-badge&logoColor=white)

- Fine-tuning small language models
- Building retrieval-augmented generation pipelines

**Local LLM Tooling**

![LM Studio](https://img.shields.io/badge/LM%20Studio-6E56CF?style=for-the-badge&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-000000?style=for-the-badge&logo=ollama&logoColor=white)
![llama.cpp](https://img.shields.io/badge/llama.cpp-4B4B4B?style=for-the-badge&logoColor=white)

- LM Studio, Ollama, and llama.cpp for running and testing models locally

---

## 💻 Development Skills

![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![Google Antigravity](https://img.shields.io/badge/Google%20Antigravity-4285F4?style=for-the-badge&logo=google&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=for-the-badge&logo=mathworks&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)

- VS Code – primary IDE for day-to-day development
- Google Antigravity – agentic coding & AI-assisted development environment
- HTML5 / CSS3 – front-end fundamentals
- Git & GitHub – version control & collaboration
- MATLAB & R – statistical computing and analysis

**Full Stack Development**

![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)

- **Front-end** – Angular and React for building UI/component-driven web apps
- **Back-end** – Node.js (Express) and Python Flask for APIs & server-side logic
- REST API design & integration between front-end and back-end services

**Databases**

![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![Azure SQL](https://img.shields.io/badge/Azure%20SQL-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)

- SQL – relational database design & querying
- PostgreSQL – open-source relational database for production workloads
- Azure SQL – managed cloud relational database on Azure
- NoSQL (MongoDB) – document-based database for flexible, unstructured data

**Other SDE / Full Stack Tools**

![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Jira](https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=jira&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)
![npm](https://img.shields.io/badge/npm-CB3837?style=for-the-badge&logo=npm&logoColor=white)

- Docker – containerization for consistent dev/deployment environments
- Postman – API testing & documentation
- Jira – agile project tracking & sprint management
- GitHub Actions / CI-CD – automated build, test & deployment pipelines
- npm/yarn – package management for JS ecosystems

---

## 📈 Observability Skills

![Grafana](https://img.shields.io/badge/Grafana-F46800?style=for-the-badge&logo=grafana&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=for-the-badge&logo=databricks&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![Power Automate](https://img.shields.io/badge/Power%20Automate-0066FF?style=for-the-badge&logo=microsoftpowerautomate&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)


- Grafana – observability dashboards & metrics monitoring
- Databricks – data engineering & analytics at scale
- Power BI – business intelligence & reporting
- Power Automate – workflow automation, including flows powered by AI agents
- Tableau – data visualization & dashboarding

<!---
TEJASsKoundinya/TEJASsKoundinya is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
